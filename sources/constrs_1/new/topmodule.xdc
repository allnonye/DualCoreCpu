set_property SLEW FAST [get_ports {CA[6]}]
set_property SLEW FAST [get_ports {CA[5]}]
set_property SLEW FAST [get_ports {CA[4]}]
set_property SLEW FAST [get_ports {CA[3]}]
set_property SLEW FAST [get_ports {CA[2]}]
set_property SLEW FAST [get_ports {CA[1]}]
set_property SLEW FAST [get_ports {CA[0]}]
set_property SLEW FAST [get_ports {AN[3]}]
set_property SLEW FAST [get_ports {AN[2]}]
set_property SLEW FAST [get_ports {AN[1]}]
set_property SLEW FAST [get_ports {AN[0]}]
set_property PACKAGE_PIN W4 [get_ports {AN[3]}]
set_property PACKAGE_PIN V4 [get_ports {AN[2]}]
set_property PACKAGE_PIN U4 [get_ports {AN[1]}]
set_property PACKAGE_PIN U2 [get_ports {AN[0]}]
set_property PACKAGE_PIN W7 [get_ports {CA[6]}]
set_property PACKAGE_PIN W6 [get_ports {CA[5]}]
set_property PACKAGE_PIN U8 [get_ports {CA[4]}]
set_property PACKAGE_PIN V8 [get_ports {CA[3]}]
set_property PACKAGE_PIN U5 [get_ports {CA[2]}]
set_property PACKAGE_PIN V5 [get_ports {CA[1]}]
set_property PACKAGE_PIN U7 [get_ports {CA[0]}]
set_property IOSTANDARD LVCMOS18 [get_ports {AN[3]}]
set_property IOSTANDARD LVCMOS18 [get_ports {AN[2]}]
set_property IOSTANDARD LVCMOS18 [get_ports {AN[1]}]
set_property IOSTANDARD LVCMOS18 [get_ports {AN[0]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[6]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[5]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[4]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[3]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[2]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[1]}]
set_property IOSTANDARD LVCMOS18 [get_ports {CA[0]}]

set_property PACKAGE_PIN W5 [get_ports clk]
create_clock -period 50.000 -name clk -waveform {0.000 5.000} [get_ports clk]

set_property IOSTANDARD LVCMOS18 [get_ports {switches[15]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[14]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[13]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[12]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[11]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[10]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[9]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[8]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[7]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[6]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[5]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[4]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[3]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[2]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[1]}]
set_property IOSTANDARD LVCMOS18 [get_ports {switches[0]}]

set_property PACKAGE_PIN T17 [get_ports {buttons[3]}]
set_property PACKAGE_PIN T18 [get_ports {buttons[2]}]
set_property PACKAGE_PIN W19 [get_ports {buttons[1]}]
set_property PACKAGE_PIN U17 [get_ports {buttons[0]}]
set_property PACKAGE_PIN U16 [get_ports {lights[0]}]
set_property PACKAGE_PIN E19 [get_ports {lights[1]}]
set_property PACKAGE_PIN U19 [get_ports {lights[2]}]
set_property PACKAGE_PIN V19 [get_ports {lights[3]}]
set_property PACKAGE_PIN W18 [get_ports {lights[4]}]
set_property PACKAGE_PIN U15 [get_ports {lights[5]}]
set_property PACKAGE_PIN U14 [get_ports {lights[6]}]
set_property PACKAGE_PIN V14 [get_ports {lights[7]}]
set_property PACKAGE_PIN V13 [get_ports {lights[8]}]
set_property PACKAGE_PIN V3 [get_ports {lights[9]}]
set_property PACKAGE_PIN W3 [get_ports {lights[10]}]
set_property PACKAGE_PIN U3 [get_ports {lights[11]}]
set_property PACKAGE_PIN P3 [get_ports {lights[12]}]
set_property PACKAGE_PIN N3 [get_ports {lights[13]}]
set_property PACKAGE_PIN P1 [get_ports {lights[14]}]
set_property PACKAGE_PIN L1 [get_ports {lights[15]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[15]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[14]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[13]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[12]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[11]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[10]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[9]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[8]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[7]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[6]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[5]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[4]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[3]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[2]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[1]}]
set_property IOSTANDARD LVCMOS18 [get_ports {lights[0]}]
set_property IOSTANDARD LVCMOS18 [get_ports {buttons[3]}]
set_property IOSTANDARD LVCMOS18 [get_ports {buttons[2]}]
set_property IOSTANDARD LVCMOS18 [get_ports {buttons[1]}]
set_property IOSTANDARD LVCMOS18 [get_ports {buttons[0]}]
set_property PACKAGE_PIN R2 [get_ports {switches[15]}]
set_property PACKAGE_PIN T1 [get_ports {switches[14]}]
set_property PACKAGE_PIN U1 [get_ports {switches[13]}]
set_property PACKAGE_PIN W2 [get_ports {switches[12]}]
set_property PACKAGE_PIN R3 [get_ports {switches[11]}]
set_property PACKAGE_PIN T2 [get_ports {switches[10]}]
set_property PACKAGE_PIN T3 [get_ports {switches[9]}]
set_property PACKAGE_PIN V2 [get_ports {switches[8]}]
set_property PACKAGE_PIN W13 [get_ports {switches[7]}]
set_property PACKAGE_PIN W14 [get_ports {switches[6]}]
set_property PACKAGE_PIN V15 [get_ports {switches[5]}]
set_property PACKAGE_PIN W15 [get_ports {switches[4]}]
set_property PACKAGE_PIN W17 [get_ports {switches[3]}]
set_property PACKAGE_PIN W16 [get_ports {switches[2]}]
set_property PACKAGE_PIN V16 [get_ports {switches[1]}]
set_property PACKAGE_PIN V17 [get_ports {switches[0]}]
set_property IOSTANDARD LVCMOS18 [get_ports clk]
set_property PACKAGE_PIN U18 [get_ports reset]
set_property IOSTANDARD LVCMOS18 [get_ports reset]
